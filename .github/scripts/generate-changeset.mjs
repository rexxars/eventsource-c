/* Auto-generates a changeset for a PR from its conventional-commit title.
 * Adapted from sanity-io/client (MIT). Invoked by
 * .github/workflows/generate-changeset.yml with the BASE branch's copy of
 * this script (never the PR's), since the workflow runs on
 * pull_request_target. */
...
const PACKAGE_NAME = 'eventsource-c'
// Paths that affect the published component - changes elsewhere (CI, host
// tests, host ports, docs) don't warrant a release. The packed artifact is
// governed by idf_component.yml's files.exclude list; keep the two in step.
...
// wrote ours: a contributor can run `npx changeset` after the bot has committed, and if we
...
// 4. Only generate a changeset when the published component is affected. Deletions count here:
